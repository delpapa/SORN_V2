...
for exp, exp_name in enumerate(os.listdir(experiment_path)):
    ...
    percent_error[exp] = 1 - float(stats.n_correct)/stats.n_output_sentences
...
plt.errorbar(N_e, new, yerr=[new_down, new_up], color='blue', alpha=0.5,
             label='New')
plt.errorbar(N_e, wrong, yerr=[wrong_down, wrong_up], color='red', alpha=0.5,
             label='Wrong')
